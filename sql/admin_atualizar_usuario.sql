...
-- (necessário pois havia uma versão com assinatura diferente)
-- ============================================================
...
-- ============================================================
-- Passo 2: Adicionar coluna admin_permissoes (se não existir)
-- ============================================================
...
CREATE FUNCTION admin_atualizar_usuario(
    p_usuario_id UUID,
...
AS $$
BEGIN
    -- Apenas executa se o chamador for o Root Admin
    IF (SELECT email FROM usuarios WHERE id = auth.uid()) != 'michelssouza@gmail.com' THEN
        RAISE EXCEPTION 'Acesso negado: apenas o Super Admin Root pode executar esta operação.';
    END IF;
...
GRANT EXECUTE ON FUNCTION admin_atualizar_usuario(UUID, TEXT, TEXT, TEXT, DATE, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, BOOLEAN, BOOLEAN, BOOLEAN, JSONB) TO authenticated;
