...
export const CATALOGO_ATALHOS = [
    ...
    { id: 'sorteio_global',   categoria: 'equipe',  label: 'Sorteio Global',   emoji: '⚡', icone: Zap,        action: (nav, set, select, eqId, setNavProps) => { if (select && eqId) select(eqId); if (setNavProps) setNavProps({ equipeId: eqId }); nav('sorteio_v4'); },                      roles: ['admin','sub_admin'], permissao: 'gerenciar_partidas' },
    { id: 'ranking_mvp',      categoria: 'equipe',  label: 'Hall da Fama',     emoji: '🎖️', icone: Trophy,    action: (nav, _, select, eqId, setNavProps) => { if (select && eqId) select(eqId); if (setNavProps) setNavProps({ equipeId: eqId }); nav('ranking_partidas'); },                 roles: [],                   permissao: null },
];
